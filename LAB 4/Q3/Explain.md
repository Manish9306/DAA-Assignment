...
2. **Select $(k - 1)$ Elements:** Use recursion or $(k - 1)$ nested loops to iterate over strictly increasing index combinations $(i_1 < i_2 < \dots < i_{k-1})$:
   - Calculate the partial sum: $\text{current\_sum} = \sum_{j=1}^{k-1} S[i_j]$.
   - Compute the complement needed: $\text{needed} = T - \text{current\_sum}$.
3. **Binary Search for Complement:**
   - Execute binary search for $\text{needed}$ in the right subarray $S[i_{k-1} + 1 \dots n - 1]$.
   - If found: **Return `TRUE`** (and the $k$ indices).
4. **Termination:** If all $\binom{n}{k-1}$ combinations are exhausted without a match, **Return `FALSE`**.
...
| **Worst-Case Time** | $O(n^{k-1} \log n)$ | Checks all $\binom{n}{k-1}$ combinations |
| **Auxiliary Space** | $O(k)$ to $O(n)$ | $O(k)$ recursion depth / index storage; $O(1)$ extra space if iterative |

### Detailed Time Breakdown
* **Initial Sort:** $O(n \log n)$
* **Outer Combinations:** Selecting $(k - 1)$ items from $n$ produces $\binom{n}{k-1} = O(n^{k-1})$ total tuples.
* **Innermost Binary Search:** Searching the remaining suffix of length $\le n$ takes $O(\log n)$ per candidate tuple.

$$\text{Total Time} = O(n \log n) + O\left(n^{k-1}\right) \times O(\log n) = \mathbf{O(n^{k-1} \log n)}$$
